...
플로우 레이아웃을 선택하면 컬렉션 뷰의 사이즈 인스팩터에 플로우 레이아웃의 메트릭을 구성하기 위한 추가 속성이 포함된다. 이러한 속성을 사용하여 셀 크기, 헤더와 푸터의 크기, 셀 간의 최소 간격 및 셀의 각 섹션 주위 여백을 구성한다. 플로우 레이아웃 메트릭의 의미에 대한 자세한 내용은 [UICollectionViewFlowLayout](https://developer.apple.com/documentation/uikit/uicollectionviewflowlayout)을 참조.
